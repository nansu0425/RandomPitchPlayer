"""
RandomPitchPlayer 릴리즈 배포용 설정 파일
이 파일을 config.py로 복사하여 릴리즈 빌드를 만드세요.
...
# 빌드 모드 설정 - 릴리즈 모드
RELEASE_MODE = True  # 릴리즈 모드 활성화

# 디버그 모드 설정 (릴리즈 모드에서는 강제로 False)
DEBUG_MODE = False
ENABLE_CONSOLE_LOGS = False      # 콘솔 로그 출력 비활성화
ENABLE_DEBUG_UI = False          # 디버그 UI 요소 비활성화
ENABLE_PERFORMANCE_ANALYSIS = False  # 성능 분석 기능 비활성화
...
# 디버깅 설정 (릴리즈 모드에서는 비활성화)
MAX_TIMING_LOGS = 0      # 로그 저장 비활성화
...
# 릴리즈 모드 실행 알림
print("RandomPitchPlayer v1.0 - 릴리즈 버전")